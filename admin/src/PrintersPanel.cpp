...
PrintersPanel::PrintersPanel(wxWindow *parent, MainFrame *frame)
  ...
  grid->Add(new wxStaticText(m_detailPanel, wxID_ANY, "Poll Interval:"), 0,
            wxALIGN_CENTER_VERTICAL);
  wxBoxSizer *pollSizer = new wxBoxSizer(wxHORIZONTAL);
  m_pollCtrl = new wxSpinCtrl(m_detailPanel, wxID_ANY, "5", wxDefaultPosition,
                              wxDefaultSize, wxSP_ARROW_KEYS, 1, 60, 5);
  m_pollCtrl->Bind(wxEVT_SPINCTRL, &PrintersPanel::OnPollChanged, this);
  pollSizer->Add(m_pollCtrl, 0);
  pollSizer->Add(new wxStaticText(m_detailPanel, wxID_ANY, " seconds"), 0,
                 wxALIGN_CENTER_VERTICAL | wxLEFT, 5);
  grid->Add(pollSizer, 0);

  grid->Add(new wxStaticText(m_detailPanel, wxID_ANY, "Print Command:"), 0,
            wxALIGN_CENTER_VERTICAL);
  ...
  wxStaticText *hint =
      new wxStaticText(m_detailPanel, wxID_ANY,
                       "Use %f as placeholder for the filename to print");
  ...
  m_list->Bind(wxEVT_LIST_ITEM_SELECTED, &PrintersPanel::OnPrinterSelected,
               this);
}
...
void PrintersPanel::ShowDetails(int index) {
  ...
  int pollVal = printer.poll_interval;
  if (pollVal < 1)
    pollVal = 1;
  if (pollVal > 60)
    pollVal = 60;
  m_pollCtrl->SetValue(pollVal);
