...
SharesPanel::SharesPanel(wxWindow *parent, MainFrame *frame)
  ...
  wxStaticText *labelName =
      new wxStaticText(m_detailPanel, wxID_ANY, "Name:", wxDefaultPosition,
                       wxSize(labelWidth, -1));
  ...
  wxStaticText *labelPath =
      new wxStaticText(m_detailPanel, wxID_ANY, "Path:", wxDefaultPosition,
                       wxSize(labelWidth, -1));
  ...
  wxStaticText *labelPass =
      new wxStaticText(m_detailPanel, wxID_ANY, "Password:", wxDefaultPosition,
                       wxSize(labelWidth, -1));
  row3->Add(labelPass, 0, wxALIGN_CENTER_VERTICAL);
  m_passwordCtrl =
  ...
  wxStaticText *labelType = new wxStaticText(
      m_detailPanel, wxID_ANY, "Default Filetype:", wxDefaultPosition,
      wxSize(labelWidth, -1));
  row4->Add(labelType, 0, wxALIGN_CENTER_VERTICAL);
  m_defaultTypeCtrl = new wxTextCtrl(m_detailPanel, wxID_ANY, "",
                                     wxDefaultPosition, wxDefaultSize);
  m_defaultTypeCtrl->SetMaxLength(3);
  m_defaultTypeCtrl->Bind(wxEVT_TEXT, &SharesPanel::OnDetailChanged, this);
  row4->Add(m_defaultTypeCtrl, 0); // No expand for small field
  formSizer->Add(row4, 0, wxEXPAND | wxBOTTOM, 8);

  detailSizer->Add(formSizer, 0, wxEXPAND | wxBOTTOM, 15);

  // Attributes
  wxStaticBoxSizer *attrBox =
      new wxStaticBoxSizer(wxVERTICAL, m_detailPanel, "Attributes");
  m_attrProtected =
      new wxCheckBox(m_detailPanel, wxID_ANY, "Protected (require password)");
...
void SharesPanel::ShowDetails(int index) {
  ...
  Layout();                // Force main panel layout
