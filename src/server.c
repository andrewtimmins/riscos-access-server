...
static void broadcast_dead_handles(sfs_handle_table *handles, sfs_net *net) {
  size_t count = 0;
  const int *dead = sfs_handles_get_dead(handles, &count);
  if (!dead || count == 0)
    return;
  ...
  sfs_net_sendto(net->rpc, pkt, 8 + count * 4, "255.255.255.255", 49171);
  sfs_log(SFS_LOG_DEBUG, "Broadcast %zu dead handles", count);
...
int sfs_server_run(sfs_config *cfg, sfs_net *net, sfs_handle_table *handles) {
  ...
  for (size_t i = 0; i < cfg->share_count; ++i) {
    ...
    if (!p || stat(p, &st) != 0) {
      sfs_log(SFS_LOG_ERROR, "share %s path missing",
  ...
  for (size_t i = 0; i < cfg->printer_count; ++i) {
    ...
    if (!p || stat(p, &st) != 0) {
      sfs_log(SFS_LOG_ERROR, "printer %s path missing",
              cfg->printers[i].name ? cfg->printers[i].name : "?");
    }
  }

  // Prepare printer spool dirs and definition files
  sfs_printers_setup(cfg);

  // Initial broadcasts
  sfs_broadcast_shares(cfg, net);
  sfs_broadcast_printers(cfg, net);
  ...
  for (;;) {
    ...
    FD_ZERO(&fds);
    FD_SET(net->rpc, &fds);
    sfs_socket maxfd = net->rpc;

    // Also listen on auth port for Access+ if enabled
    if (cfg->server.access_plus && net->auth != (sfs_socket)-1) {
      FD_SET(net->auth, &fds);
      if (net->auth > maxfd)
        maxfd = net->auth;
    }
    ...
    if (net->freeway != (sfs_socket)-1) {
      FD_SET(net->freeway, &fds);
      if (net->freeway > maxfd)
        maxfd = net->freeway;
    }
    ...
    if (ready > 0) {
      ...
      if (FD_ISSET(net->rpc, &fds)) {
        ...
        ssize_t n = sfs_net_recvfrom(net->rpc, buf, sizeof(buf), addr,
                                     sizeof(addr), &port);
        if (n > 0) {
          sfs_log(SFS_LOG_PROTOCOL, "RPC %zd bytes from %s:%u", n, addr, port);
          sfs_rpc_handle(buf, (size_t)n, addr, port, cfg, net, handles, &auth);
        }
      }

      // Handle Access+ auth packets
      if (cfg->server.access_plus && net->auth != (sfs_socket)-1 &&
          FD_ISSET(net->auth, &fds)) {
        unsigned char buf[1024];
        char addr[64];
        unsigned short port = 0;
        ssize_t n = sfs_net_recvfrom(net->auth, buf, sizeof(buf), addr,
                                     sizeof(addr), &port);
        if (n > 0) {
          sfs_log(SFS_LOG_PROTOCOL, "Auth %zd bytes from %s:%u", n, addr, port);
          sfs_accessplus_handle(buf, (size_t)n, addr, port, cfg, net, &auth);
        }
      }

      // Handle Freeway packets (client announcements)
      if (net->freeway != (sfs_socket)-1 && FD_ISSET(net->freeway, &fds)) {
        unsigned char buf[1024];
        char addr[64];
        unsigned short port = 0;
        ssize_t n = sfs_net_recvfrom(net->freeway, buf, sizeof(buf), addr,
                                     sizeof(addr), &port);
        if (n > 0) {
          sfs_log(SFS_LOG_PROTOCOL, "Freeway %zd bytes from %s:%u", n, addr,
                  port);
    ...
    if (cfg->server.broadcast_interval > 0 &&
        (now - last_bcast) >= cfg->server.broadcast_interval) {
      sfs_broadcast_shares(cfg, net);
      sfs_broadcast_printers(cfg, net);
