...
int ras_server_run(ras_config *cfg, ras_net *net, ras_handle_table *handles) {
  ...
  for (size_t i = 0; i < cfg->share_count; ++i) {
    ...
    if (!p || stat(p, &st) != 0) {
      ras_log(RAS_LOG_ERROR, "share %s path missing",
  ...
  for (size_t i = 0; i < cfg->printer_count; ++i) {
    ...
    if (!p || stat(p, &st) != 0) {
      ras_log(RAS_LOG_ERROR, "printer %s path missing",
  ...
  for (;;) {
    ...
    if (ready > 0) {
      ...
      if (FD_ISSET(net->rpc, &fds)) {
        ...
        ssize_t n = ras_net_recvfrom(net->rpc, buf, sizeof(buf), addr,
                                     sizeof(addr), &port);
        if (n > 0) {
      ...
      if (cfg->server.access_plus && net->auth != (ras_socket)-1 &&
        ...
        char addr[64];
        unsigned short port = 0;
        ssize_t n = ras_net_recvfrom(net->auth, buf, sizeof(buf), addr,
                                     sizeof(addr), &port);
        if (n > 0) {
      ...
      if (net->freeway != (ras_socket)-1 && FD_ISSET(net->freeway, &fds)) {
        ...
        char addr[64];
        unsigned short port = 0;
        ssize_t n = ras_net_recvfrom(net->freeway, buf, sizeof(buf), addr,
                                     sizeof(addr), &port);
        if (n > 0) {
          ras_log(RAS_LOG_PROTOCOL, "Freeway %zd bytes from %s:%u", n, addr,
                  port);
