...
static const struct {
  ...
  uint32_t type;
    ...
    {"txt", 0xFFF},    {"text", 0xFFF}, {"bas", 0xFFB},  {"c", 0xFFD},
    {"h", 0xFFD},      {"s", 0xFFF},    {"o", 0xFFE},    {"pdf", 0xADF},
    {"png", 0xB60},    {"jpg", 0xC85},  {"jpeg", 0xC85}, {"gif", 0x695},
    {"zip", 0xA91},    {"html", 0xFAF}, {"htm", 0xFAF},  {"css", 0xF79},
    {"js", 0xF81},     {"json", 0xF79}, {"xml", 0xF80},  {"csv", 0xDFE},
    {"sprite", 0xFF9}, {"draw", 0xAFF}, {"ff9", 0xFF9},  {"aff", 0xAFF},
    {NULL, 0}};

// Standard RISC OS filename mapping (fallback if no suffix/extension)
static const struct {
  const char *name;
  uint32_t type;
} std_file_map[] = {{"!Run", 0xFEB},       // Obey
                    {"!Boot", 0xFEB},      // Obey
                    {"!Sprites", 0xFF9},   // Sprite
                    {"!Sprites22", 0xFF9}, // Sprite
                    {"!Sprites11", 0xFF9}, // Sprite
                    {"!Help", 0xFFF},      // Text
                    {NULL, 0}};
...
uint32_t sfs_filetype_from_ext(const char *filename, const sfs_config *cfg) {
  ...
  // fallback: check standard filenames (case-insensitive)
  for (int j = 0; std_file_map[j].name; ++j) {
    if (strcasecmp(filename, std_file_map[j].name) == 0) {
      return std_file_map[j].type;
    }
  }
